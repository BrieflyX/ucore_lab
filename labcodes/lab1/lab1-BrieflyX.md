...
编程题因为代码中已经给出了很详尽的注释，思路清晰，故与答案的实现区别不大。而前面的原理部分，主要是回答的没有答案完整，例如对makefile中指令参数的理解和解说答案中都给出的十分详细，而因为对于makefile的语法不熟悉其中有一些符号不太看得明白。在调试的环节，gdbinit的配置更细致一些，例如有set architecture i8086等命令。

## 实验中的知识点

1. 主引导记录MBR的格式问题，实验中给出了相应的练习，原理课中也提到了对应的内容。只是原理课中对于MBR的描述要更加详尽，包括启动代码和硬盘分区表等。而实验中并没有解释前510字节的内容。

2. bootloader程序的作用，代码分为汇编和C的2部分，对于其完成的工作实验中给出了一个练习，原理课中也叙述了它的作用。

## 原理课中的知识点

原理课中提到了中断、异常和系统调用，而实验中仅仅有对IDT初始化和处理时钟中断的描述，可能在后续的实验中还会进一步完善。